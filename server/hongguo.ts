...
export const API_BASE = 'https://api5-normal-sinfonlineb.fqnovel.com';
export const API_HOSTS = [
  'api5-normal-sinfonlineb.fqnovel.com',
  'api5-normal-hl.fqnovel.com',
];
export const UA = 'com.phoenix.read/71532 (Linux; U; Android 9; SM-N9860; Build/PQ3A.190705.10241111;tt-ok/3.12.13.20)';
export const VIDEO_REFERER = 'https://novelquickapp.com/';
...
/**
 * Call Hongguo API with multi-host fallback and retry
 */
export async function apiCall(apiPath: string, body: unknown, retries = 2): Promise<any> {
  const q = { ...COMMON_QUERY, _rticket: Date.now().toString() };
  let lastError: any = null;

  for (const host of API_HOSTS) {
    const headers = { ...HEADERS, Host: host };
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        // If 502, 503, 504 or network/timeout error, try fallback host immediately
  ...
  throw lastError || new Error('API Hồng Quả không phản hồi, vui lòng thử lại sau');
}

/**
 * Extract series_id from raw string, mixed text, or share URL
 */
export async function resolveSeriesId(shareUrl: string): Promise<string> {
  const trimmed = String(shareUrl || '').trim();

  // 1. If already pure digits (15-22 digits)
  if (/^\d{15,22}$/.test(trimmed)) {
    return trimmed;
  }

  // 2. Look for explicit series_id/video_series_id/book_id/video_id parameters in text
  let m = trimmed.match(/(?:video_series_id|series_id|book_id|item_id|video_id)[=/:]([0-9]{15,22})/i);
  if (m) return m[1];

  m = trimmed.match(/(?:series_id%22%3A%22|video_id%22%3A%22)([0-9]{15,22})/i);
  if (m) return m[1];

  // 3. Look for path patterns like /page/7664958856774044697 or /detail/7664958856774044697
  m = trimmed.match(/\/(?:page|detail|series|book|video)\/([0-9]{15,22})/i);
  if (m) return m[1];

  // 4. Extract URL from mixed text if any
  const urlMatch = trimmed.match(/https?:\/\/[^\s]+/i);
  const targetUrl = urlMatch ? urlMatch[0] : null;

  // 5. If there is no URL, but there is a 15-22 digit sequence anywhere in the text
  const directDigitMatch = trimmed.match(/\b([0-9]{15,22})\b/);
  if (!targetUrl && directDigitMatch) {
    return directDigitMatch[1];
  }

  // 6. If an HTTP URL is found, request it to resolve redirects and metadata
  if (targetUrl) {
    try {
      const res = await axios.get(targetUrl, {
        headers: {
          'User-Agent':
            'Mozilla/5.0 (Linux; Android 9; SM-N9860) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.152 Mobile Safari/537.36',
          'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
        },
        timeout: 20000,
        maxRedirects: 10,
        validateStatus: () => true, // Don't throw on 502/404 so we can inspect redirect or body text
      });

      const finalUrl = (res.request as any)?.res?.responseUrl || res.config?.url || targetUrl;
      m = finalUrl.match(/video_series_id=(\d+)/) || finalUrl.match(/series_id=(\d+)/);
      if (m) return m[1];
      ...
      m =
        bodyText.match(/video_series_id=(\d+)/) ||
        bodyText.match(/"video_id"\s*:\s*"(\d+)"/) ||
        bodyText.match(/"series_id"\s*:\s*"(\d+)"/) ||
        bodyText.match(/series_id%22%3A%22(\d+)%22/);
      if (m) return m[1];

      // Check if bodyText contains a 19-digit number
      const bodyDigitMatch = bodyText.match(/\b(7[0-9]{17,20})\b/);
      if (bodyDigitMatch) return bodyDigitMatch[1];

      if (res.status >= 500) {
        throw new Error('Máy chủ liên kết chia sẻ tạm thời phản hồi lỗi 502/503. Vui lòng nhập trực tiếp ID phim (ví dụ: 7650820508048444441)');
      }
    } catch (err: any) {
      if (directDigitMatch) {
        return directDigitMatch[1];
      }
      throw new Error(err.message || 'Không thể phân tích liên kết短剧');
    }
  }

  // 7. Fallback to any digit sequence in input
  if (directDigitMatch) {
    return directDigitMatch[1];
  }

  throw new Error('Không thể tìm thấy ID phim từ liên kết hoặc văn bản. Vui lòng nhập trực tiếp dãy số ID phim');
}

/**
 * Fetch episode list for a series
 */
export async function fetchEpisodeList(seriesId: string): Promise<SeriesData> {
  const body = {
  ...
  const eps: Episode[] = vl.map((item: any) => ({
    vid: String(item.vid),
...
/**
 * Score video definition stream
 */
function streamScore(v: any) {
  ...
  return { defNum, pixels: w * h, bitrate, defStr };
}

/**
 * Parse video_model JSON for highest quality stream and decryption key
 */
function parseModelVideo(vm: any): [string | null, string | null, string | null, string | null] {
  if (typeof vm !== 'string') return [null, null, null, null];
  let vmj: any;
  try {
    vmj = JSON.parse(vm);
  } catch {
  ...
  const cmp = (a: any, b: any) => {
    ...
    if (a.codecRank !== b.codecRank) return b.codecRank - a.codecRank;
  ...
  for (const v of vl) {
    ...
    if (!bestScore || cmp(s, bestScore) > 0) {
      bestScore = s;
      best = [v.main_url, ei.spade_a || null, codec, s.defStr || '1080p'];
    }
  }

  return best || [null, null, null, null];
}

/**
 * Fetch play stream URL and spade_a encryption metadata for a single video ID
 */
export async function fetchPlayUrlSingle(vid: string): Promise<PlayInfo> {
  const body = {
  ...
  try {
    ...
    const [url, spadeA, codec, definition] = parseModelVideo(item.video_model);
    return { url, spadeA, codec, definition: definition || undefined };
  } catch {
...
/**
 * FFmpeg compatible Base64 decoder
 */
function avBase64Decode(s: string): Buffer {
...
/**
 * Derive 16-byte AES-128 key from spade_a string
 */
export function deriveKey(spadeA: string): Buffer | null {
 ...
 * AES-128-ECB
 */
function aes128EcbEncrypt(key16: Buffer, data: Buffer): Buffer {
...
/**
 * Decrypt sample using CENC-AES-CTR
 */
function decryptSample(key: Buffer, nonce: Buffer, cipher: Buffer): Buffer {
 ...
 */
function parseBoxes(data: Buffer, start: number, end: number): Box[] {
  const boxes: Box[] = [];
  let off = start;
  while (off + 8 <= end) {
...
/**
 * Rebuild moov box to strip encryption atoms (encv/enca -> hvc1/mp4a)
 */
function rebuildMoov(data: Buffer, moovOff: number, moovSize: number, top: Box[]): Buffer {
  ...
  function* walkChildren(box: Box): Generator<Box> {
    for (const c of box.children || []) {
      yield c;
  ...
  const patches: Record<number, Buffer> = {};
  for (const b of walkChildren(tree)) {
    if (b.typ === 'stsd') {
      ...
      for (let i = 0; i < count; i++) {
        const esize = data.readUInt32BE(p);
  ...
  function serialize(box: Box): Buffer {
    if (!box.children) {
...
/**
 * Decrypt encrypted MP4 Buffer or File using CENC-AES-CTR and return Buffer
 */
export function decryptMp4Buffer(srcBuffer: Buffer, key: Buffer): Buffer {
  const data = Buffer.from(srcBuffer);
  const total = data.length;
  const top = parseBoxes(data, 0, total);
  const moov = top.find((b) => b.typ === 'moov');
  if (!moov) throw new Error('no moov box found in MP4');

  function getBox(boxes: Box[] | null, typ: string) {
    return (boxes || []).find((b) => b.typ === typ);
  }

  function* walkBoxes(boxes: Box[]): Generator<Box> {
  ...
  for (const stbl of stbls) {
    ...
    const chunkSpc: Record<number, number> = {};
...
/**
 * Decrypt MP4 file from disk to disk
 */
export function decryptMp4File(srcPath: string, dstPath: string, key: Buffer): void {
